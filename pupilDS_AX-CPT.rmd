...
pupilAXCPT = pupilAXCPT[!is.na(pupilAXCPT$PCA) & 
                        !is.na(pupilAXCPT$rMCI_cons_v2pe) &
...
lme.dprime.loads.MCI <- lapply(loads, function(x) {
    lmer(residPCA ~ CPTDPRIME_V2_nasp + apoe4 + c.age_v2 + site_v2rev + 
                    Device + acamedbin + c.DSFMAX_V2_nasp + rMCI_cons_4grp  + 
                    (1 | case), 
...
# Plot predicted values
dfDSFRAWpred = sjp.int(lme.facLoad.dprime.MCI, type="eff", int.term="facLoad:c.DSFMAX_V2_nasp", 
               swap.pred=T, mdrt.values = "quart", print.plot=F)$data.list[[1]]
...
Plot pupil dilation by MCI group
```{r, echo=FALSE}
...
# Plot predicted values
dfMCIpred = sjp.int(lme.facLoad.dprime.MCI, type="eff", int.term="facLoad:rMCI_cons_4grp", 
               swap.pred=T, print.plot=F)$data.list[[1]]
pMCIpred = ggplot(data=dfMCIpred, aes(x=x, y=y, color=grp, group=grp)) + 
          geom_line(size=1) +
          geom_errorbar(width=.2,size=1, aes(ymin=conf.low,ymax=conf.high)) +
          ylab(expression(paste("Predicted ", Delta, " Pupil Diameter"))) + xlab("Load") +
          scale_color_brewer(name="MCI Status",palette='Set1') + 
